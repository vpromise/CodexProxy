...
func TestRuntimeAuthSyncHook_SynchronousModelAndSchedulerRestoration(t *testing.T) {
	...
	resp, errExec := manager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: "gpt-5.5"}, cliproxyexecutor.Options{})
	...
	if _, errExecDisabled := manager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: "gpt-5.5"}, cliproxyexecutor.Options{}); errExecDisabled == nil {
	...
	if !hasAstra {
		t.Fatalf("expected gpt-5.5 in restored models: %+v", modelsAfterEnable)
	}

	// Verify scheduler selection is restored for the re-enabled auth
	respAfterEnable, errExecEnable := manager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: "gpt-5.5"}, cliproxyexecutor.Options{})
	...
	if _, errQuotaExec := manager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: "gpt-5.5"}, cliproxyexecutor.Options{}); errQuotaExec == nil {
...
func TestRuntimeAuthSync_FileSnapshotEnqueuedThenRequestExecutionThenConsume(t *testing.T) {
	...
	// File synthesis never supplies manager-owned runtime versions.
	fileSnapshotAuth.RegistrationEpoch, fileSnapshotAuth.Generation = 0, 0
...
func TestEndToEndAuthFileReplacement_RestoresModelsInV1ModelsWithoutRestart(t *testing.T) {
	...
	initialContent := fmt.Sprintf(`{"type":"codex","email":"user@example.com","access_token":"token-old","id_token":%q,"prefix":%q}`, teamIDToken, fileName)
	...
	cfg.APIKeys = []string{"lifecycle-test-key"}
	...
	req.Header.Set("Authorization", "Bearer lifecycle-test-key")
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
	...
	hasModel := func(modelID string) bool {
		for _, m := range modelsResp.Data {
			if id, _ := m["id"].(string); id == fileName+"/"+modelID {
				return true
			}
		}
		return false
	}
	teamModels := []string{"gpt-5.5", "gpt-5.6-sol", "gpt-5.6-terra", "codex-auto-review"}
	...
	newContent := fmt.Sprintf(`{"type":"codex","email":"user@example.com","access_token":"token-new","id_token":%q,"prefix":%q}`, teamIDToken, fileName)
	...
	service.coreManager.RegisterExecutor(&syncTestExecutor{})
	resp, errExec := service.coreManager.Execute(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: fileName + "/gpt-5.5"}, cliproxyexecutor.Options{})
