...
func (m *Manager) ReconcileRegistryModelStates(ctx context.Context, authID string) {
	...
	if errPersist := m.persist(context.Background(), snapshot); errPersist != nil {
		logEntryWithRequestID(ctx).WithField("auth_index", snapshot.Index).Warnf("failed to persist reconciled auth state: %v", errPersist)
	}
	m.publishAuthModelStates(snapshot, regEpoch)
