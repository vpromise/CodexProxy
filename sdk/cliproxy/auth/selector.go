...
func authAvailabilityBlock(auth *Auth, now time.Time) (bool, blockReason, time.Time) {
	...
	// Aggregated model quotas must not block an otherwise available credential.
	if len(auth.ModelStates) > 0 && auth.Quota.Reason != "credential_quota" && !auth.Unavailable {
		quotaExceeded = false
	}
	blocked, reason, next := availabilityBlock(auth.Unavailable, quotaExceeded, auth.NextRetryAfter, auth.Quota.NextRecoverAt, now)
