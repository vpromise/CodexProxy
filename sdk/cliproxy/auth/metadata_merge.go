...
func IsAuthTokenPayloadKey(key string) bool {
	...
	case "access_token", "accesstoken", "refresh_token", "refreshtoken", "id_token", "idtoken", "session_id",
...
func MergeRefreshedAuth(base, current, updated *Auth) *Auth {
	...
	hasNewConcurrentError := authAvailabilityChanged(base, current)
	...
	if updated.ModelStates != nil && (base == nil || base.availabilityEpoch == current.availabilityEpoch) {
	...
	if updated.Metadata != nil {
		for k, v := range updated.Metadata {
			...
			if changedByExecutor {
				// Apply executor changes only when the current value still matches the base.
				if !changedByUser {
					merged.Metadata[k] = v
				}
			}
...
// cloneAuthMetadataValue isolates the JSON containers that executors may update
// in place. A shallow clone would also mutate the base of a three-way merge.
func cloneAuthMetadataValue(value any) any {
	switch v := value.(type) {
	case map[string]any:
		out := make(map[string]any, len(v))
		for key, item := range v {
			out[key] = cloneAuthMetadataValue(item)
		}
		return out
	...
	case []any:
		out := make([]any, len(v))
		for i, item := range v {
			out[i] = cloneAuthMetadataValue(item)
		}
		return out
	case []string:
