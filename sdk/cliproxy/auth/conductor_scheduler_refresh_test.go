...
func TestManager_PrepareRequestAuth_DoesNotRollbackConcurrentlyRefreshedToken(t *testing.T) {
	...
	// Credential replacement invalidates identity metadata discovered with the old token.
	if gotProjID, _ := finalAuth.Metadata["project_id"].(string); gotProjID != "" {
		t.Fatalf("stale preparation changed project_id: %q", gotProjID)
	}
}
