...
func TestCloudflareOriginFailurePreservesExistingCooldown(t *testing.T) {
	withQuotaCooldownEnabled(t)
	for _, model := range []string{"", "test-model"} {
		for status := 520; status <= 526; status++ {
			t.Run(fmt.Sprintf("model=%s/status=%d", model, status), func(t *testing.T) {
				manager := NewManager(nil, nil, nil)
				auth, errRegister := manager.Register(context.Background(), &Auth{ID: t.Name(), Provider: "codex", Status: StatusActive})
				if errRegister != nil {
					t.Fatal(errRegister)
				}
				longHint := 5 * time.Minute
				shortHint := time.Second
				result := Result{AuthID: auth.ID, Provider: "codex", Model: model, Error: &Error{HTTPStatus: status, Message: "Cloudflare origin unavailable"}, RetryAfter: &longHint}
				manager.MarkResult(context.Background(), result)
				before, _ := manager.GetByID(auth.ID)
				result.RetryAfter = &shortHint
				manager.MarkResult(context.Background(), result)
				after, _ := manager.GetByID(auth.ID)
				deadlineBefore, deadlineAfter := before.NextRetryAfter, after.NextRetryAfter
				if model != "" {
					deadlineBefore, deadlineAfter = before.ModelStates[model].NextRetryAfter, after.ModelStates[model].NextRetryAfter
				}
				if !deadlineBefore.Equal(deadlineAfter) || deadlineAfter.Before(time.Now().Add(4*time.Minute)) {
					t.Fatalf("origin failure shortened cooldown: before=%v after=%v", deadlineBefore, deadlineAfter)
				}
				if after.Quota.Exceeded || (model != "" && after.ModelStates[model].Quota.Exceeded) {
					t.Fatal("origin failure became a quota or challenge cooldown")
				}
			})
		}
	}
