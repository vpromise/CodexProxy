...
func TestManager_RemoveAndReRegister_ClearsSchedulerTombstone(t *testing.T) {
	...
	// Replacements register their models after runtime removal cleared the old client.
...
func TestManager_Load_MonotonicEpochAssignment_AndRemovalTombstones(t *testing.T) {
	...
	// Runtime versions are assigned by the manager, ignoring disk values.
	if loadedAuth1.RegistrationEpoch != 1 {
		t.Fatalf("expected auth1 RegistrationEpoch == 1, got %d", loadedAuth1.RegistrationEpoch)
	}
	if loadedAuth1.Generation != 1 {
		t.Fatalf("expected auth1 Generation == 1, got %d", loadedAuth1.Generation)
	}
	// Both records start at the first manager-owned registration.
	if loadedAuth2.RegistrationEpoch != 1 {
		t.Fatalf("expected auth2 RegistrationEpoch == 1, got %d", loadedAuth2.RegistrationEpoch)
	}
	...
	// Reload assigns a new runtime epoch.
	if reloadedAuth1 == nil || reloadedAuth1.RegistrationEpoch != 2 {
		t.Fatalf("expected reloaded auth1 with epoch 2, got %#v", reloadedAuth1)
	}
	...
	// The removed record receives a newer tombstone.
	if epochAuth2Tombstone != 2 {
		t.Fatalf("expected auth2 tombstone epoch == 2, got %d", epochAuth2Tombstone)
	}

	// Scheduler must have tombstone for auth2 with epoch 2
	manager.scheduler.mu.Lock()
	metaAuth2 := manager.scheduler.authGenerations[authID2]
	manager.scheduler.mu.Unlock()
	if metaAuth2.epoch != 2 || metaAuth2.generation != 0 {
		t.Fatalf("expected scheduler tombstone for auth2 with epoch 2, generation 0, got %#v", metaAuth2)
	}
	...
	// 3. A snapshot from the removed registration cannot undo the tombstone.
	staleAuth2 := &Auth{
		ID:                authID2,
		Provider:          provider,
		Status:            StatusActive,
		RegistrationEpoch: loadedAuth2.RegistrationEpoch,
		Generation:        999,
		UpdatedAt:         time.Now(),
	}
