...
//
// Classify the interrupted transport as a server error while retaining the
// fork's official-Codex response.failed event contract.
func TestCodexIncompleteStreamIsNotTypedAsInvalidRequest(t *testing.T) {
	...
	if got := detail["type"]; got == "invalid_request_error" {
		t.Fatalf("error.type = %q for a truncated stream: the request was well formed, and this incorrectly blames the client request", got)
	}
	if got := detail["type"]; got != "server_error" {
		t.Fatalf("error.type = %v, want %q for a transport failure", got, "server_error")
	}
}
