...
func TestListAuthFiles_IncludesProjectIDFromManager(t *testing.T) {
	...
	authDir := t.TempDir()
	fileName := "customoauth-user@example.com-project-a.json"
	filePath := filepath.Join(authDir, fileName)
	if errWrite := os.WriteFile(filePath, []byte(`{"type":"custom-oauth","email":"user@example.com","project_id":"project-a"}`), 0o600); errWrite != nil {
...
func TestListAuthFilesFromDisk_IncludesProjectID(t *testing.T) {
	...
	authDir := t.TempDir()
	filePath := filepath.Join(authDir, "customoauth-user@example.com-project-a.json")
	if errWrite := os.WriteFile(filePath, []byte(`{"type":"custom-oauth","email":"user@example.com","project_id":"project-a"}`), 0o600); errWrite != nil {
		t.Fatalf("failed to write auth file: %v", errWrite)
	}
