...
func convertOpenAIRequestToClaude(modelName string, inputRawJSON []byte, stream, preserveEmptyThinkingBlocks bool) []byte {
	...
	if toolChoice := root.Get("tool_choice"); toolChoice.Exists() && toolChoice.IsObject() && toolChoice.Get("type").String() == "allowed_tools" {
		isAllowedTools = true
		toolList := toolChoice.Get("allowed_tools.tools")
		if !toolList.Exists() {
			toolList = toolChoice.Get("tools")
		}
		for _, t := range toolList.Array() {
	...
	// Unknown selections must not fall back to implicit permission to use tools.
	// Tool choice mapping from OpenAI format to Claude Code format.
	if isAllowedTools {
		if len(anthropicTools) == 0 || (allowedMode != "auto" && allowedMode != "required") {
