...
func TestStreamingTool_MultiChunkTruncatedWithTrailingUsageEmitsMaxTokens(t *testing.T) {
	...
	if countByType(events, "message_delta") != 1 || countByType(events, "message_stop") != 1 {
		t.Fatalf("expected one terminal delta and stop: %+v", events)
	}
	for _, event := range events {
		if event.Type == "message_delta" {
			if gjson.Get(event.Payload, "usage.input_tokens").Int() != 10 || gjson.Get(event.Payload, "usage.output_tokens").Int() != 400 {
				t.Fatalf("truncated tool lost trailing usage: %s", event.Payload)
			}
		}
	}
}

func TestStreamingTool_CompleteArgumentsDoNotOverrideLength(t *testing.T) {
	events := runStream(t, streamReq,
		`{"id":"c1","model":"m","choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_1","type":"function","function":{"name":"get_time","arguments":"{}"}}]},"finish_reason":null}]}`,
		`{"id":"c1","model":"m","choices":[{"index":0,"delta":{},"finish_reason":"length"}]}`,
	)
	if got := lastStopReason(events); got != "max_tokens" {
