...
func TestClaudeCCHSigningEnabled(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "official API key default", apiKey: "key-123", origin: anthropicOrigin, want: false},
		{name: "official API key opt-in", apiKey: "key-123", cliFingerprint: true, origin: anthropicOrigin, want: true},
		{name: "gateway API key opt-in", apiKey: "key-123", cliFingerprint: true, origin: gatewayOrigin, want: false},
		{name: "anthropic host over http opt-in", apiKey: "key-123", cliFingerprint: true, origin: "http://api.anthropic.com/v1/messages", want: false},
		{name: "anthropic host explicit 443 opt-in", apiKey: "key-123", cliFingerprint: true, origin: "https://api.anthropic.com:443/v1/messages", want: true},
		{name: "anthropic lookalike host opt-in", apiKey: "key-123", cliFingerprint: true, origin: "https://api.anthropic.com.evil.example/v1/messages", want: false},
		...
		{name: "Claude OAuth", apiKey: "sk-ant-oat-custom", origin: anthropicOrigin, want: true},
		{name: "Claude OAuth custom gateway", apiKey: "sk-ant-oat-custom", origin: gatewayOrigin, want: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := claudeCCHSigningEnabled(tt.apiKey, tt.cliFingerprint, tt.origin); got != tt.want {
