...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	diagnosticsBeforeInjection := gjson.GetBytes(body, "diagnostics")
	...
	if isProbeOrHelper {
		...
		if diagnosticsInjectedByCPA && !touchedPayloadPaths["diagnostics"] {
			if diagnosticsBeforeInjection.Exists() {
				body, _ = sjson.SetRawBytes(body, "diagnostics", []byte(diagnosticsBeforeInjection.Raw))
			} else {
