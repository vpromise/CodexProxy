...
// claudeCode2_1_258WireHeaderOrder extends the local 2.1.252 HTTP/1.1 capture
// with the first-party request ID reported by upstream for 2.1.258. It omits
// transport-owned names (Connection, Host, Accept-Encoding, Content-Length)
// and User-Agent. Custom upstreams need not include x-client-request-id.
...
var claudeCode2_1_258WireHeaderOrder = []string{
	"Accept",
	"Authorization",
	"Content-Type",
...
func TestApplyClaudeHeaders_WireCasingMatchesRealClient(t *testing.T) {
	...
	want := claudeCode2_1_258WireHeaderOrder
...
// name can move back into claudeCode2_1_258WireHeaderOrder.
...
func TestClaudeWireHeaderOrderIsBytewiseSorted(t *testing.T) {
	sorted := append([]string(nil), claudeCode2_1_258WireHeaderOrder...)
	sort.Strings(sorted)
	for i := range sorted {
		if sorted[i] != claudeCode2_1_258WireHeaderOrder[i] {
			t.Fatalf("captured order is not a bytewise sort at %d: %q vs %q", i, claudeCode2_1_258WireHeaderOrder[i], sorted[i])
		}
	}
}
