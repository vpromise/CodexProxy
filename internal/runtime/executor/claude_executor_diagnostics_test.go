...
func TestClaudeExecutorPayloadReclassificationPreservesDiagnosticsAndContinuity(t *testing.T) {
	for _, stream := range []bool{false, true} {
		for _, probeToNormal := range []bool{false, true} {
			t.Run(fmt.Sprintf("stream=%t/probe-to-normal=%t", stream, probeToNormal), func(t *testing.T) {
				var captured [][]byte
				transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
					...
					response := `{"id":"msg_reclassified","type":"message","model":"claude-sonnet-5","role":"assistant","content":[{"type":"text","text":"ok"}],"usage":{"input_tokens":1,"output_tokens":1}}`
					if stream {
						response = "data: {\"type\":\"message_start\",\"message\":" + response + "}\n\ndata: {\"type\":\"message_stop\"}\n\n"
					}
					return &http.Response{StatusCode: http.StatusOK, Header: http.Header{"Request-Id": {"req_reclassified"}}, Body: io.NopCloser(strings.NewReader(response)), Request: req}, nil
				})
				ctx := context.WithValue(t.Context(), "cliproxy.roundtripper", http.RoundTripper(transport))
				params := map[string]any{"max_tokens": 1}
				payload := []byte(`{"model":"claude-sonnet-5","messages":[{"role":"user","content":"quota"}],"max_tokens":1000,"diagnostics":{"caller":"keep"}}`)
				if probeToNormal {
					params["max_tokens"] = 1000
					params["diagnostics"] = map[string]any{"operator": "keep"}
					payload, _ = sjson.SetBytes(payload, "max_tokens", 1)
				}
				cfg := &config.Config{Payload: config.PayloadConfig{Override: []config.PayloadRule{{
					Models: []config.PayloadModelRule{{Name: "claude-sonnet-5"}}, Params: params,
				}}}}
				auth := &cliproxyauth.Auth{ID: uuid.NewString(), Attributes: map[string]string{"api_key": "sk-ant-oat-reclassification"}, Metadata: claudeOAuthTestMetadata()}
				executor := NewClaudeExecutor(cfg)
				options := cliproxyexecutor.Options{SourceFormat: sdktranslator.FormatClaude, Metadata: map[string]any{cliproxyexecutor.ExecutionSessionMetadataKey: uuid.NewString()}}
				for range 2 {
				...
				for _, body := range captured {
					if probeToNormal {
						if got := gjson.GetBytes(body, "diagnostics.operator").String(); got != "keep" {
							t.Fatalf("payload diagnostics lost: %s", body)
						}
					} else {
						if got := gjson.GetBytes(body, "diagnostics.caller").String(); got != "keep" {
							t.Fatalf("caller diagnostics lost: %s", body)
						}
						if prev, prompt := helps.ExtractClaudeBillingTags(body); prev != "" || prompt != "" {
							t.Fatalf("probe carries continuity tags: %s", body)
						}
					}
				}
				if probeToNormal {
					if prev, _ := helps.ExtractClaudeBillingTags(captured[1]); prev != "req_reclassified" {
						t.Fatalf("reclassified request did not commit continuity: %s", captured[1])
					}
				}
			})
		}
