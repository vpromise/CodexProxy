...
func TestStreamUsageBufferClaudeSparseThinkingAndExplicitZero(t *testing.T) {
	var buffer StreamUsageBuffer
	buffer.ObserveClaudeStream([]byte(`data: {"type":"message_start","message":{"usage":{"input_tokens":10,"cache_read_input_tokens":20,"cache_creation_input_tokens":30,"output_tokens":1}}}`))
	buffer.ObserveClaudeStream([]byte(`data: {"type":"message_delta","usage":{"output_tokens":8,"output_tokens_details":{"thinking_tokens":5}}}`))
	buffer.ObserveClaudeStream([]byte(`data: {"type":"message_delta","usage":{"output_tokens":12}}`))
	detail, ok := buffer.Detail()
	if !ok || detail.InputTokens != 10 || detail.ReasoningTokens != 5 || detail.OutputTokens != 12 || detail.TotalTokens != 72 {
		t.Fatalf("sparse thinking usage = %+v, present = %v", detail, ok)
	}
	buffer.ObserveClaudeStream([]byte(`data: {"type":"message_delta","usage":{"cache_read_input_tokens":0,"cache_creation_input_tokens":0,"output_tokens":0,"output_tokens_details":{"thinking_tokens":0}}}`))
	detail, _ = buffer.Detail()
	if detail.InputTokens != 10 || detail.OutputTokens != 0 || detail.ReasoningTokens != 0 || detail.CachedTokens != 0 || detail.TotalTokens != 10 {
		t.Fatalf("explicit zero must replace previous counters: %+v", detail)
	}
	for _, line := range []string{`event: message_stop`, `data: {"type":"message_stop"}`, `data: {"usage":null}`, `data: {"usage":[]}`, `data: malformed`} {
		buffer.ObserveClaudeStream([]byte(line))
	}
	after, _ := buffer.Detail()
	if after != detail {
		t.Fatalf("non-usage events changed accumulated counters: %+v -> %+v", detail, after)
	}
}
