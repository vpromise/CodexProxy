...
describe('stripJsonExtension', () => {
  ...
  test('matches the extension case-insensitively', () => {
    expect(stripJsonExtension('Claude-User@Example.com.JSON')).toBe(
      'Claude-User@Example.com'
    );
  });

  test('leaves an extension-less runtime-only id untouched', () => {
    expect(stripJsonExtension('codex-channel-a')).toBe('codex-channel-a');
  });
  ...
  test('trims surrounding whitespace', () => {
    expect(stripJsonExtension('  claude-1712345678901.json  ')).toBe('claude-1712345678901');
  });
});
...
describe('deriveAuthFileIdentity', () => {
  ...
  test('treats a whitespace-only email as absent', () => {
    const identity = deriveAuthFileIdentity(
      authFile({ name: 'claude-1712345678901.json', email: '   ' })
    );
    expect(identity.kind).toBe('fileName');
    expect(identity.primary).toBe('claude-1712345678901');
  });

  test('treats a non-string email as absent (index signature guard)', () => {
    const identity = deriveAuthFileIdentity(
      authFile({ name: 'claude-1712345678901.json', email: 123 as unknown as string })
    );
    expect(identity.kind).toBe('fileName');
    expect(identity.primary).toBe('claude-1712345678901');
  });
  ...
  test('falls back to the file name and drops the duplicate secondary row', () => {
    expect(
      deriveAuthFileIdentity(authFile({ name: 'claude-1712345678901.json', type: 'claude' }))
    ).toEqual({
      primary: 'claude-1712345678901',
      kind: 'fileName',
      secondary: null,
      fullName: 'claude-1712345678901.json',
    });
  });
  ...
  test('uses the project id when there is no email', () => {
    expect(
      deriveAuthFileIdentity(
        authFile({ name: 'codex-my-proj.json', type: 'codex', projectId: 'my-proj' })
      )
    ).toEqual({
      primary: 'my-proj',
      kind: 'projectId',
      secondary: 'codex-my-proj',
      fullName: 'codex-my-proj.json',
    });
  });
  ...
  test('never surfaces the account field — it can be a raw API key', () => {
    ...
    expect(identity.kind).toBe('fileName');
    expect(identity.primary).toBe('codex-apikey');
  ...
  test('suppresses the secondary row for runtime-only entries whose name is the account', () => {
    expect(
      deriveAuthFileIdentity(
        authFile({
          name: 'codex-channel-a',
          type: 'codex',
          email: 'codex-channel-a',
          runtimeOnly: true,
        })
      ).secondary
    ).toBeNull();
