...
describe('matchesAuthFileSearch', () => {
  ...
  test('matches the type and the provider', () => {
    expect(search(authFile({ type: 'claude' }), 'claud')).toBe(true);
    expect(search(authFile({ type: undefined, provider: 'codex' }), 'code')).toBe(true);
  });

  test('finds a Claude credential by account email even though its name carries none', () => {
    expect(
      search(
        authFile({ name: 'claude-1712345678901.json', type: 'claude', email: 'user@example.com' }),
        'user@example'
      )
    ).toBe(true);
  });
  ...
  test('matches the project id', () => {
    expect(search(authFile({ name: 'codex-x.json', projectId: 'my-proj' }), 'my-proj')).toBe(true);
  });

  test('searches the email on the wildcard path too', () => {
    expect(
      search(authFile({ name: 'claude-1712345678901.json', email: 'user@example.com' }), 'user@*com')
    ).toBe(true);
  });
  ...
  test('never matches the account field — it can be a raw API key', () => {
    expect(
      search(
        authFile({ name: 'codex-apikey.json', account: 'sk-live-abcd', account_type: 'api_key' }),
        'sk-live'
      )
    ).toBe(false);
  });
...
describe('sortAuthFiles', () => {
  test("'default' orders by provider then name", () => {
    const files = [
      authFile({ name: 'b.json', provider: 'claude' }),
      authFile({ name: 'c.json', type: 'codex', provider: undefined }),
      authFile({ name: 'a.json', provider: 'claude' }),
    ];
  ...
  test("'az' mixes account primaries and file-name fallbacks coherently", () => {
    const files = [
      authFile({ name: 'claude-9.json', type: 'claude' }),
      authFile({ name: 'zzz.json', email: 'aaa@example.com' }),
    ];
    expect(sortAuthFiles(files, 'az').map((file) => file.name)).toEqual([
      'zzz.json',
