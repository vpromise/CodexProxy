...
describe('provider key counts', () => {
  test('counts only supported provider configurations', () => {
    const counts = getProviderKeyCounts({
      codexApiKeys: [{ apiKey: 'codex-key' }],
      claudeApiKeys: [{ apiKey: 'claude-key' }],
      openaiCompatibility: [{ name: 'compatible', baseUrl: 'https://example.com/v1' }],
    });

    expect(counts).toEqual({ codex: 1, claude: 1, openai: 1 });
  });
});

describe('providerLabel', () => {
  test('uses the brand spelling for known providers', () => {
    expect(providerLabel('codex', 'Unattributed')).toBe('Codex');
    expect(providerLabel('claude', 'Unattributed')).toBe('Claude');
    expect(providerLabel('openai-compatibility', 'Unattributed')).toBe('OpenAI Compatible');
  });
