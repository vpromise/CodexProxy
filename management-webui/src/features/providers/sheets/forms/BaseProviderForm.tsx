...
function buildInitialForm(
  ...
  return {
    ...
    websockets: brand === 'codex' ? cfg.websockets === true : undefined,
    cloak: isClaudeLikeBrand(brand)
      ? {
...
export function BaseProviderForm({
  ...
  /**
   * The candidate catalog combines discovered models with names already in the form.
   * It may be empty before discovery, so the picker must still work as a rule editor.
   */
  ...
  const supportsDisableCooling = true;
        ...
        {descriptor.supportsTestModel ? (
          <div className={styles.field}>
            <label className={styles.label} htmlFor={`${fid}-testModel`}>
              ...
              {brand === 'codex' || isClaudeLikeBrand(brand) ? (
                <span className={styles.labelHint}>
                  {' '}
