...
import type { OpenAIProviderConfig, ProviderKeyConfig, ApiKeyEntry, ModelAlias } from '@/types';
...
export function removeLatestProviderRecord(
  latestItems: unknown[],
  isTarget: (record: Record<string, unknown>, index: number) => boolean
): unknown[] {
  const targetIndex = latestItems.findIndex(
  ...
  return latestItems.filter((_, index) => index !== targetIndex);
}
...
export const providersApi = {
  ...
  updateOpenAIProviderDisabled: (name: string, index: number, disabled: boolean) =>
    mutateLatestProviderList('openai-compatibility', (latestItems) =>
      replaceLatestProviderRecord(
        latestItems,
        (record, currentIndex) => currentIndex === index && matchesOpenAIProvider(record, name),
        { disabled },
        (raw, payload) => mergeKnownFields(raw, payload, [])
      )
    ),

  deleteOpenAIProvider: (name: string, index: number) =>
    mutateLatestProviderList('openai-compatibility', (latestItems) =>
